...
| The graph after a reindex | `make graph TENANT=` (`GRAPH_ARGS="--source hr_policy_2026.md"` for one document's graph, deterministically; the corpus otherwise) | `graph_built` with nodes and edges; only chunks whose `chunk_hash` changed are re-extracted (`graph_extractions/`), the rest is cached |
